podTemplate(
    ...
    node(POD_LABEL) {
        ...
        stage('Build') {
            ...
            sh "${mvn}/bin/mvn clean package -Ddocker-registry=turaco-registry -Ddockerfile.skip=true"
            container('podman') {
                echo "Podman Build ing..."
                sh "podman login -u $HARBOR_USER -p $HARBOR_PASSWORD $IMAGE_REPO_NAME"
                sh "podman build -t $IMAGE_REPO_NAME:$ARGO_APPLICATION-latest --build-arg JAR_FILE=$SERVICE_NAME\\.jar -f ./src/main/docker ./"
